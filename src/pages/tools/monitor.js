...
function Monitor() {
	...
	const getDate = () => dayjs(new Date()).format('DD/MM/YYYY HH:mm:ss.SSS');
		...
		ws.onmessage = (event) => {
			...
			if (schema) {
				...
				setMessages((prevMessages) => [
					...prevMessages.slice(0, LIMIT_MESSAGES),
					{
						name: schema?.name || 'UNNAMED_OPERATION',
						timestamp: getDate(),
						...message,
						dataParsed: data,
						data: message.data,
			...
			setMessages((prevMessages) => [
				...prevMessages.slice(0, LIMIT_MESSAGES),
				{timestamp: getDate(), ...message, dataParsed: message.data},
			]);
		};
						...
						>
							<Box flexDirection="row" display="flex" justifyContent="space-between">
								<Typography fontSize={12} color="primary">
									...
									{message?.name && (
										<>
											- [<b style={{color: 'white'}}>{message.name}</b>]
										</>
									)}
								</Typography>
							...
							<Box>
								...
								<Box>
									<Code>{JSON.stringify(message.dataParsed, null, 1)}</Code>
								</Box>
							</Box>
						</Box>
					))}
